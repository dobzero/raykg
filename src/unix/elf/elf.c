...
void elf_needed_print_deps(const elf_needed_t *deps) {
    for (size_t i=0; deps && i<deps->list_size; i++) {
...
unix_elf_bin_t * unix_elf_open(const char * path) {
    ...
    return unix_elf_open_fp(fp);
}
unix_elf_bin_t * unix_elf_open_fp(FILE *fp) {
    unix_elf_bin_t * elf = calloc(1, sizeof(unix_elf_bin_t));
    elf->fp = fp;
    elf->string_table = elf_strtab_new(0);


    Elf64_Ehdr * hdr = malloc(sizeof(Elf64_Ehdr)+sizeof(Elf64_Shdr));
    fread(hdr, sizeof(Elf64_Ehdr), 1, elf->fp);
    ...
    if (sections[3]&&sections[4]) {
        elf->deps=elf_needed_new();
    }

    if (hdr->e_shentsize) {
        fseek(elf->fp, (long)hdr->e_shoff, SEEK_SET);
        Elf64_Shdr * slist = realloc(exchange((void**)&hdr, nullptr), sizeof(Elf64_Shdr)*hdr->e_shentsize);
        if (slist)
            elf->sections=slist;

        elf->sections_count = fread(slist, sizeof(Elf64_Shdr), sections[0], elf->fp);
        for (size_t i=0; i<elf->sections_count; i++) {
            if (slist[i].sh_type==SHT_STRTAB || i==sections[1]) {
                elf_strtab_emplace(elf->string_table, elf->fp, slist[i].sh_offset, slist[i].sh_size);
            }
        }

        fseek(elf->fp, (long)sections[3], SEEK_SET);
        Elf64_Phdr * plist = calloc(sizeof(Elf64_Phdr), sections[4]);
        const size_t plist_count = fread(plist, sizeof(Elf64_Phdr), sections[4], elf->fp);

        for (size_t i=0; i<plist_count; i++) {
            if (plist[i].p_type!=PT_DYNAMIC) {
                continue;
            }
            fseek(elf->fp, (long)plist[i].p_offset, SEEK_SET);
            const size_t dyn_count = plist[i].p_filesz/sizeof(Elf64_Dyn);
            Elf64_Dyn dyn_list[dyn_count];
            fread(dyn_list, sizeof(Elf64_Dyn), dyn_count, elf->fp);

            for (size_t j = 0; j<dyn_count; j++) {
                elf_needed_emplace(elf->deps, elf->string_table, &dyn_list[j]);
            }
        }
...
void unix_elf_print(const unix_elf_bin_t *elf) {
    printf("elf deps: \n");
    elf_needed_print_deps(elf->deps);
}

void unix_elf_close(unix_elf_bin_t *elf) {
    fclose(elf->fp);
    elf_strtab_delete(elf->string_table);
    if (elf->deps)
        elf_needed_delete(elf->deps);
    free(elf->sections);
    free(elf);
}
